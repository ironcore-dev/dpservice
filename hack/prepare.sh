...
process_switchdev_mode() {
	...
	if ! devlink dev eswitch set pci/$pf mode switchdev; then
		log "cannot set eswitch mode, setting VFs to 0"
		echo 0 > /sys/bus/pci/devices/$pf/sriov_numvfs
		err "cannot set eswitch mode"
	fi
	if [[ "$MULTIPORT" == "true" ]]; then
		log "enabling multiport eswitch mode"
...
function get_pattern() {
	...
	if [ -z "$pattern" ]; then
		err "cannot determine the pattern for $dev"
...
function make_config() {
	...
	echo "ipv6 $(get_ipv6)"
	if [[ "$MULTIPORT" == "true" ]]; then
		echo "a-pf0 ${devs[0]},class=rxq_cqe_comp_en=0,rx_vec_en=1,dv_flow_en=2,dv_esw_en=1,fdb_def_rule_en=1,representor=pf0-1vf0-$[$actualvfs-1]"
		echo "mesw-mode"
	else
...
# Unless there are more arguments, this parser should suffice
if [ $# -eq 1 ]; then
	if [ $1 = "--multiport" ]; then
		MULTIPORT=true
	fi
