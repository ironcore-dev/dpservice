...
uint16_t dp_grpc_init_responder(struct dp_grpc_responder *responder, struct rte_mbuf *req_mbuf)
{
	...
	responder->rep_max_size = req_mbuf->buf_len - req_mbuf->data_off - offsetof(dp_reply, messages);
...
int dp_grpc_alloc_reply(struct dp_grpc_responder *responder)
{
	...
	if (responder->repcount >= RTE_DIM(responder->replies)) {
		DPGRPC_LOG_WARNING("gRPC response array is full, truncated", DP_LOG_GRPCREQUEST(responder->req.type));
		return DP_ERROR;
	}
	m_new = rte_pktmbuf_alloc(responder->mempool);
	if (!m_new) {
		DPGRPC_LOG_WARNING("gRPC response mbuf allocation failed", DP_LOG_GRPCREQUEST(responder->req.type));
		return DP_ERROR;
	}
	...
	rep_new->type = responder->req.type;
...
void dp_grpc_send_response(struct dp_grpc_responder *responder, int grpc_ret)
{
	...
	rte_pktmbuf_mtod(responder->replies[0], dp_reply *)->err_code = grpc_ret;
	...
	if (sent != responder->repcount) {
		DPGRPC_LOG_WARNING("Not all gRPC responses were queued", DP_LOG_GRPCREQUEST(responder->req.type));
