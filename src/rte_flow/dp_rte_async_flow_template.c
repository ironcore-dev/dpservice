...
static int dp_create_pf_default_async_rule_templates(uint16_t port_id) {
	...
	ret = dp_rte_async_create_pattern_template(port_id, pattern, &pattern_template_attr, &(port->default_async_rules.async_templates.pattern_templates[DP_ASYNC_RULE_TYPE_DEFAULT_ISOLATION]));
	if (DP_FAILED(ret))
		return DP_ERROR;
	...
	ret = dp_rte_async_create_actions_template(port_id, action, action_mask, &action_template_attr, &(port->default_async_rules.async_templates.action_templates[DP_ASYNC_RULE_TYPE_DEFAULT_ISOLATION]));
	if (DP_FAILED(ret))
		return DP_ERROR;
...
static int dp_create_pf_default_async_rule_templates_test(uint16_t port_id) {
	...
	ret = dp_rte_async_create_pattern_template(port_id, pattern, &pattern_template_attr, &(port->default_async_rules.async_templates.pattern_templates[DP_ASYNC_RULE_TYPE_DEFAULT_ISOLATION_TEST]));
	if (DP_FAILED(ret))
		return DP_ERROR;
	...
	ret = dp_rte_async_create_actions_template(port_id, action, action_mask, &action_template_attr, &(port->default_async_rules.async_templates.action_templates[DP_ASYNC_RULE_TYPE_DEFAULT_ISOLATION_TEST]));
	if (DP_FAILED(ret))
		return DP_ERROR;
...
int dp_create_pf_async_rte_rule_templates(uint16_t port_id) {
	...
	if (DP_FAILED(dp_create_pf_default_async_rule_templates_test(port_id))) {
		DPS_LOG_ERR("Failed to create pf async rte rule templates", DP_LOG_PORTID(port_id));
		return DP_ERROR;
	}
	pf_pattern_action_template_counter ++;
	...
	if (DP_FAILED(dp_rte_async_create_template_tables(port_id, pf_pattern_action_template_counter))) {
		DPS_LOG_ERR("Failed to create pf async rte rule template table", DP_LOG_PORTID(port_id));
		return DP_ERROR;
	}

	pf_pattern_action_template_counter = 0;
