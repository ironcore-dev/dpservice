...
int dp_create_pf_async_isolation_templates(struct dp_port *port) {
	...
	struct dp_port_async_template *template = &port->default_async_rules.async_templates[DP_PORT_ASYNC_TEMPLATE_PF_ISOLATION];
...
int dp_create_pf_async_isolation_rules(struct dp_port *port)
{
	...
	const struct dp_port_async_template *templates = port->default_async_rules.async_templates;
