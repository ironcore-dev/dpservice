...
int dp_create_pf_async_isolation_templates(struct dp_port *port)
{
	...
	static const struct rte_flow_item pattern[] = {
		...
		{	.type = RTE_FLOW_ITEM_TYPE_IPV6,
			.mask = &dp_flow_item_ipv6_dst_pfx68_mask,
		},
...
static struct rte_flow *dp_create_pf_async_isolation_rule(uint16_t port_id, struct rte_flow_template_table *template_table)
{
...
int dp_create_pf_async_isolation_rules(struct dp_port *port)
{
	...
	if (!flow) {
		...
		port->default_async_rules.default_flows[DP_PORT_ASYNC_FLOW_ISOLATE_PREFIX] = flow;
		rule_count++;
	}

	// add more rules here and increase rules_required appropriately
	...
	// only now we can fail due to previous errors
