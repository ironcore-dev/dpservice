										...
										uint32_t install_to_port)
{
	...
	dp_set_raw_encap_action(&sub_action[sub_action_cnt++], encap_action, encaped_mirror_hdr, sizeof(struct rte_ether_hdr) + sizeof(struct rte_ipv6_hdr));
...
int dp_install_default_rule_in_monitoring_group(uint16_t port_id, bool is_on)
{
	...
	// uint8_t raw_encap_hdr[sizeof(struct rte_ether_hdr) + sizeof(struct rte_ipv6_hdr) + sizeof(struct rte_udp_hdr)];
	uint8_t raw_encap_hdr[sizeof(struct rte_ether_hdr) + sizeof(struct rte_ipv6_hdr)];
