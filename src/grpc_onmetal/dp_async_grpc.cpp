...
CallOnmetalState BaseCallOnmetal::HandleRpc()
{
	switch (call_state_) {
	case REQUEST: {
...
int BaseCallOnmetal::WriteRequest(struct dpgrpc_request *request)
{
	struct rte_mbuf *m;
	int ret;
...
int SingleReplyCallOnmetal::HandleReply()
{
...
int MultiReplyCallOnmetal::HandleReply()
{
	struct rte_mbuf *m;
	struct dpgrpc_reply *reply;
	uint8_t is_chained;
...
int NoIpcCallOnmetal::WriteRequest(__rte_unused struct dpgrpc_request *request)
{
	// no IPC, so no request needs to be filled here, do nothing
	return DP_OK;
}

int NoIpcCallOnmetal::HandleReply()
{
...
const char* InitializeCallOnmetal::FillRequest(__rte_unused struct dpgrpc_request* request)
{
	DPGRPC_LOG_INFO("Initializing");
	return NULL;
}
void InitializeCallOnmetal::ParseReply(__rte_unused struct dpgrpc_reply* reply)
{
	GRPCServiceOnmetal::GetInstance()->SetInitStatus(true);
	reply_.set_uuid(service_->GetUUID());
}

const char* CheckInitializedCallOnmetal::FillRequest(__rte_unused struct dpgrpc_request* request)
{
	return NULL;
}
void CheckInitializedCallOnmetal::ParseReply(__rte_unused struct dpgrpc_reply* reply)
{
	// reply is actually NULL here
	reply_.set_uuid(service_->GetUUID());
}

const char* GetVersionCallOnmetal::FillRequest(struct dpgrpc_request* request)
{
...
void GetVersionCallOnmetal::ParseReply(struct dpgrpc_reply* reply)
{
...
const char* CreateInterfaceCallOnmetal::FillRequest(struct dpgrpc_request* request)
{
	...
	if (!GrpcConvOnmetal::IsInterfaceIdValid(request_.interface_id()))
		return "Invalid interface_id";
	request->add_iface.vni = request_.vni();
	if (!GrpcConvOnmetal::StrToIpv4(request_.ipv4_config().primary_address(), &request->add_iface.ip4_addr))
		return "Invalid ipv4_config.primary_address";
	if (!GrpcConvOnmetal::StrToIpv6(request_.ipv6_config().primary_address(), &request->add_iface.ip6_addr))
	...
	if (!request_.pxe_config().next_server().empty()) {
		...
		if (!GrpcConvOnmetal::StrToDpAddress(request_.pxe_config().next_server(), &request->add_iface.pxe_addr,
	...
	if (!GrpcConvOnmetal::StrToPreferredUnderlay(request_.preferred_underlay_route(), &request->add_iface.ul_addr6))
...
void CreateInterfaceCallOnmetal::ParseReply(struct dpgrpc_reply* reply)
{
	VirtualFunction *vf;
...
const char* DeleteInterfaceCallOnmetal::FillRequest(struct dpgrpc_request* request)
{
...
void DeleteInterfaceCallOnmetal::ParseReply(__rte_unused struct dpgrpc_reply* reply)
{
}

const char* GetInterfaceCallOnmetal::FillRequest(struct dpgrpc_request* request)
{
...
void GetInterfaceCallOnmetal::ParseReply(struct dpgrpc_reply* reply)
{
	Interface *grpc_iface;

	grpc_iface = new Interface();
	GrpcConvOnmetal::DpToGrpcInterface(&reply->iface, grpc_iface);
	reply_.set_allocated_interface(grpc_iface);
}

const char* ListInterfacesCallOnmetal::FillRequest(__rte_unused struct dpgrpc_request* request)
{
	DPGRPC_LOG_INFO("Listing interfaces");
	return NULL;
}
void ListInterfacesCallOnmetal::ParseReply(struct dpgrpc_reply* reply)
{
	...
	FOREACH_MESSAGE(dp_iface, reply) {
		...
		GrpcConvOnmetal::DpToGrpcInterface(dp_iface, grpc_iface);
	}
}


const char* CreatePrefixCallOnmetal::FillRequest(struct dpgrpc_request* request)
{
	...
	if (SNPRINTF_FAILED(request->add_pfx.iface_id, request_.interface_id()))
		return "Invalid interface_id";
	if (!GrpcConvOnmetal::GrpcToDpAddress(request_.prefix().ip(), &request->del_pfx.addr))
	...
	if (!GrpcConvOnmetal::StrToPreferredUnderlay(request_.preferred_underlay_route(), &request->add_pfx.ul_addr6))
		return "Invalid preferred_underlay_route";
	return NULL;
}
void CreatePrefixCallOnmetal::ParseReply(struct dpgrpc_reply* reply)
{
	char strbuf[INET6_ADDRSTRLEN];
...
const char* DeletePrefixCallOnmetal::FillRequest(struct dpgrpc_request* request)
{
...
void DeletePrefixCallOnmetal::ParseReply(__rte_unused struct dpgrpc_reply* reply)
{
}

const char* ListPrefixesCallOnmetal::FillRequest(struct dpgrpc_request* request)
{
...
void ListPrefixesCallOnmetal::ParseReply(struct dpgrpc_reply* reply)
{
	...
	FOREACH_MESSAGE(route, reply) {
		...
		GrpcConvOnmetal::DpToGrpcAddress(&route->pfx_addr, pfx_ip);
...
const char* CreateRouteCallOnmetal::FillRequest(struct dpgrpc_request* request)
{
	...
	if (!GrpcConvOnmetal::GrpcToDpAddress(request_.route().prefix().ip(), &request->add_route.pfx_addr))
		return "Invalid route.prefix.ip";
	if (!GrpcConvOnmetal::GrpcToDpAddress(request_.route().nexthop_address(), &request->add_route.trgt_addr))
		return "Invalid route.nexthop_address";
	return NULL;
}
void CreateRouteCallOnmetal::ParseReply(__rte_unused struct dpgrpc_reply* reply)
{
}

const char* DeleteRouteCallOnmetal::FillRequest(struct dpgrpc_request* request)
{
...
void DeleteRouteCallOnmetal::ParseReply(__rte_unused struct dpgrpc_reply* reply)
{
}

const char* ListRoutesCallOnmetal::FillRequest(struct dpgrpc_request* request)
{
...
void ListRoutesCallOnmetal::ParseReply(struct dpgrpc_reply* reply)
{
	struct dpgrpc_route *route;
	Route *grpc_route;
	IpAddress *nh_ip;
	...
	FOREACH_MESSAGE(route, reply) {
		...
		nh_ip = new IpAddress();
		GrpcConvOnmetal::DpToGrpcAddress(&route->trgt_addr, nh_ip);
...
const char* CreateVipCallOnmetal::FillRequest(struct dpgrpc_request* request)
{
	...
	if (SNPRINTF_FAILED(request->add_vip.iface_id, request_.interface_id()))
		return "Invalid interface_id";
	if (!GrpcConvOnmetal::GrpcToDpAddress(request_.vip_ip(), &request->add_vip.addr))
		return "Invalid vip_ip";
	if (!GrpcConvOnmetal::StrToPreferredUnderlay(request_.preferred_underlay_route(), &request->add_vip.ul_addr6))
		return "Invalid preferred_underlay_route";
	return NULL;
}
void CreateVipCallOnmetal::ParseReply(struct dpgrpc_reply* reply)
{
	char strbuf[INET6_ADDRSTRLEN];
...
const char* DeleteVipCallOnmetal::FillRequest(struct dpgrpc_request* request)
{
...
void DeleteVipCallOnmetal::ParseReply(__rte_unused struct dpgrpc_reply* reply)
{
}

const char* GetVipCallOnmetal::FillRequest(struct dpgrpc_request* request)
{
...
void GetVipCallOnmetal::ParseReply(struct dpgrpc_reply* reply)
{
	IpAddress *vip_ip;
	char strbuf[INET6_ADDRSTRLEN];

	vip_ip = new IpAddress();
	GrpcConvOnmetal::DpToGrpcAddress(&reply->vip.addr, vip_ip);
...
const char* CreateNatCallOnmetal::FillRequest(struct dpgrpc_request* request)
{
	...
	if (SNPRINTF_FAILED(request->add_nat.iface_id, request_.interface_id()))
		return "Invalid interface_id";
	if (!GrpcConvOnmetal::GrpcToDpAddress(request_.nat_ip(), &request->add_nat.addr))
	...
	if (!GrpcConvOnmetal::StrToPreferredUnderlay(request_.preferred_underlay_route(), &request->add_nat.ul_addr6))
		return "Invalid preferred_underlay_route";
	return NULL;
}
void CreateNatCallOnmetal::ParseReply(struct dpgrpc_reply* reply)
{
	char strbuf[INET6_ADDRSTRLEN];
...
const char* GetNatCallOnmetal::FillRequest(struct dpgrpc_request* request)
{
...
void GetNatCallOnmetal::ParseReply(struct dpgrpc_reply* reply)
{
	IpAddress *nat_ip;
	char strbuf[INET6_ADDRSTRLEN];

	nat_ip = new IpAddress();
	GrpcConvOnmetal::DpToGrpcAddress(&reply->nat.addr, nat_ip);
...
const char* DeleteNatCallOnmetal::FillRequest(struct dpgrpc_request* request)
{
...
void DeleteNatCallOnmetal::ParseReply(__rte_unused struct dpgrpc_reply* reply)
{
}

const char* ListLocalNatsCallOnmetal::FillRequest(struct dpgrpc_request* request)
{
	DPGRPC_LOG_INFO("Listing local Nats",
					DP_LOG_IPV4STR(request_.nat_ip().address().c_str()));
	if (!GrpcConvOnmetal::GrpcToDpAddress(request_.nat_ip(), &request->list_localnat))
		return "Invalid nat_ip";
	return NULL;
}
void ListLocalNatsCallOnmetal::ParseReply(struct dpgrpc_reply* reply)
{
	...
	FOREACH_MESSAGE(nat, reply) {
		...
		GrpcConvOnmetal::DpToGrpcAddress(&nat->natted_ip, natted_ip);
		...
		nat_ip = new IpAddress();
		GrpcConvOnmetal::DpToGrpcAddress(&nat->addr, nat_ip);
		nat_entry->set_allocated_actual_nat_ip(nat_ip);
	}
}

const char* CreateNeighborNatCallOnmetal::FillRequest(struct dpgrpc_request* request)
{
	...
	if (!GrpcConvOnmetal::GrpcToDpAddress(request_.nat_ip(), &request->add_neighnat.addr))
		return "Invalid nat_ip";
	if (request_.min_port() > UINT16_MAX)
		return "Invalid min_port";
	if (request_.max_port() > UINT16_MAX)
	...
	if (!GrpcConvOnmetal::StrToIpv6(request_.underlay_route(), &request->add_neighnat.neigh_addr6))
		return "Invalid underlay_route";
	return NULL;
}
void CreateNeighborNatCallOnmetal::ParseReply(__rte_unused struct dpgrpc_reply* reply)
{
}

const char* DeleteNeighborNatCallOnmetal::FillRequest(struct dpgrpc_request* request)
{
...
void DeleteNeighborNatCallOnmetal::ParseReply(__rte_unused struct dpgrpc_reply* reply)
{
}

const char* ListNeighborNatsCallOnmetal::FillRequest(struct dpgrpc_request* request)
{
...
void ListNeighborNatsCallOnmetal::ParseReply(struct dpgrpc_reply* reply)
{
	struct dpgrpc_nat *nat;
	NatEntry *nat_entry;
	IpAddress *nat_ip;
...
const char* CreateLoadBalancerCallOnmetal::FillRequest(struct dpgrpc_request* request)
{
	uint8_t proto;
	uint16_t port;
	...
	if (!GrpcConvOnmetal::GrpcToDpAddress(request_.loadbalanced_ip(), &request->add_lb.addr))
		return "Invalid loadbalanced_ip";
	if (request_.loadbalanced_ports_size() >= DP_LB_MAX_PORTS)
		return "Too many loadbalanced_ports";
	if (!GrpcConvOnmetal::StrToPreferredUnderlay(request_.preferred_underlay_route(), &request->add_lb.ul_addr6))
		return "Invalid preferred_underlay_route";
...
void CreateLoadBalancerCallOnmetal::ParseReply(struct dpgrpc_reply* reply)
{
	char strbuf[INET6_ADDRSTRLEN];
...
const char* DeleteLoadBalancerCallOnmetal::FillRequest(struct dpgrpc_request* request)
{
...
void DeleteLoadBalancerCallOnmetal::ParseReply(__rte_unused struct dpgrpc_reply* reply)
{
}

const char* GetLoadBalancerCallOnmetal::FillRequest(struct dpgrpc_request* request)
{
...
void GetLoadBalancerCallOnmetal::ParseReply(struct dpgrpc_reply* reply)
{
	LbPort *lb_port;
	IpAddress *lb_ip;
	char strbuf[INET6_ADDRSTRLEN];
	...
	lb_ip = new IpAddress();
	GrpcConvOnmetal::DpToGrpcAddress(&reply->lb.addr, lb_ip);
...
const char* ListLoadBalancersCallOnmetal::FillRequest(__rte_unused struct dpgrpc_request* request)
{
	DPGRPC_LOG_INFO("Listing loadbalancers");
	return NULL;
}
void ListLoadBalancersCallOnmetal::ParseReply(struct dpgrpc_reply* reply)
{
	...
	FOREACH_MESSAGE(dp_lb, reply) {
		...
		// TODO GetLoadBalancerCallOnmetal should use this in the future
		// (it's currently not as to not break backward-compatibility)
		GrpcConvOnmetal::DpToGrpcLoadBalancer(dp_lb, grpc_lb);
	}
}



const char* CreateLoadBalancerTargetCallOnmetal::FillRequest(struct dpgrpc_request* request)
{
	...
	if (SNPRINTF_FAILED(request->add_lbtrgt.lb_id, request_.loadbalancer_id()))
		return "Invalid loadbalancer_id";
	if (!GrpcConvOnmetal::GrpcToDpAddress(request_.target_ip(), &request->add_lbtrgt.addr))
		return "Invalid target_ip";
	return NULL;
}
void CreateLoadBalancerTargetCallOnmetal::ParseReply(__rte_unused struct dpgrpc_reply* reply)
{
}

const char* DeleteLoadBalancerTargetCallOnmetal::FillRequest(struct dpgrpc_request* request)
{
	...
	if (SNPRINTF_FAILED(request->del_lbtrgt.lb_id, request_.loadbalancer_id()))
		return "Invalid loadbalancer_id";
	if (!GrpcConvOnmetal::GrpcToDpAddress(request_.target_ip(), &request->del_lbtrgt.addr))
		return "Invalid target_ip";
	return NULL;
}
void DeleteLoadBalancerTargetCallOnmetal::ParseReply(__rte_unused struct dpgrpc_reply* reply)
{
}

const char* ListLoadBalancerTargetsCallOnmetal::FillRequest(struct dpgrpc_request* request)
{
...
void ListLoadBalancerTargetsCallOnmetal::ParseReply(struct dpgrpc_reply* reply)
{
	...
	FOREACH_MESSAGE(lb_target, reply) {
		...
		GrpcConvOnmetal::DpToGrpcAddress(&lb_target->addr, target_ip);
	}
}


const char* CreateLoadBalancerPrefixCallOnmetal::FillRequest(struct dpgrpc_request* request)
{
	...
	if (SNPRINTF_FAILED(request->add_lbpfx.iface_id, request_.interface_id()))
		return "Invalid interface_id";
	if (!GrpcConvOnmetal::GrpcToDpAddress(request_.prefix().ip(), &request->add_lbpfx.addr))
		return "Invalid prefix.ip";
	if (request_.prefix().length() > UINT8_MAX)
		return "Invalid prefix.length";
	request->add_lbpfx.length = (uint8_t)request_.prefix().length();
	if (!GrpcConvOnmetal::StrToPreferredUnderlay(request_.preferred_underlay_route(), &request->add_lbpfx.ul_addr6))
		return "Invalid preferred_underlay_route";
	return NULL;
}
void CreateLoadBalancerPrefixCallOnmetal::ParseReply(struct dpgrpc_reply* reply)
{
	char strbuf[INET6_ADDRSTRLEN];
...
const char* DeleteLoadBalancerPrefixCallOnmetal::FillRequest(struct dpgrpc_request* request)
{
	...
	if (SNPRINTF_FAILED(request->del_lbpfx.iface_id, request_.interface_id()))
		return "Invalid interface_id";
	if (!GrpcConvOnmetal::GrpcToDpAddress(request_.prefix().ip(), &request->del_lbpfx.addr))
		return "Invalid prefix.ip";
	if (request_.prefix().length() > UINT8_MAX)
		return "Invalid prefix.length";
...
void DeleteLoadBalancerPrefixCallOnmetal::ParseReply(__rte_unused struct dpgrpc_reply* reply)
{
}

const char* ListLoadBalancerPrefixesCallOnmetal::FillRequest(struct dpgrpc_request* request)
{
...
void ListLoadBalancerPrefixesCallOnmetal::ParseReply(struct dpgrpc_reply* reply)
{
	struct dpgrpc_route *route;
	Prefix *pfx;
	IpAddress *pfx_ip;
...
const char* CreateFirewallRuleCallOnmetal::FillRequest(struct dpgrpc_request* request)
{
	...
	if (!GrpcConvOnmetal::GrpcToDpAddress(grpc_rule.source_prefix().ip(), &dp_rule->src_ip))
		return "Invalid source_prefix.ip";
	if (grpc_rule.source_prefix().ip().ipver() == IpVersion::IPV4) {
		if (!GrpcConvOnmetal::Ipv4PrefixLenToMask(grpc_rule.source_prefix().length(), &dp_rule->src_mask))
			return "Invalid source_prefix.length";
	} else {
		if (!GrpcConvOnmetal::Ipv6PrefixLenToMask(grpc_rule.source_prefix().length(), &dp_rule->src_mask))
	...
	if (!GrpcConvOnmetal::GrpcToDpAddress(grpc_rule.destination_prefix().ip(), &dp_rule->dest_ip))
		return "Invalid destination_prefix.ip";
	if (grpc_rule.destination_prefix().ip().ipver() == IpVersion::IPV4) {
		if (!GrpcConvOnmetal::Ipv4PrefixLenToMask(grpc_rule.destination_prefix().length(), &dp_rule->dest_mask))
			return "Invalid destination_prefix.length";
	} else {
		if (!GrpcConvOnmetal::Ipv6PrefixLenToMask(grpc_rule.destination_prefix().length(), &dp_rule->dest_mask))
			return "Invalid ip6 destination_prefix.length";
	}
	if (!GrpcConvOnmetal::GrpcToDpFwallDirection(grpc_rule.direction(), &dp_rule->dir))
		return "Invalid direction";
	if (!GrpcConvOnmetal::GrpcToDpFwallAction(grpc_rule.action(), &dp_rule->action))
	...
	case ProtocolFilter::kTcpFieldNumber:
		...
		if (!GrpcConvOnmetal::GrpcToDpFwallPort(grpc_filter.tcp().src_port_lower(), &dp_ports->src_port.lower))
			return "Invalid tcp.src_port_lower";
		if (!GrpcConvOnmetal::GrpcToDpFwallPort(grpc_filter.tcp().dst_port_lower(), &dp_ports->dst_port.lower))
			return "Invalid tcp.dst_port_lower";
		if (!GrpcConvOnmetal::GrpcToDpFwallPort(grpc_filter.tcp().src_port_upper(), &dp_ports->src_port.upper))
			return "Invalid tcp.src_port_upper";
		if (!GrpcConvOnmetal::GrpcToDpFwallPort(grpc_filter.tcp().dst_port_upper(), &dp_ports->dst_port.upper))
	...
	case ProtocolFilter::kUdpFieldNumber:
		...
		if (!GrpcConvOnmetal::GrpcToDpFwallPort(grpc_filter.udp().src_port_lower(), &dp_ports->src_port.lower))
			return "Invalid udp.src_port_lower";
		if (!GrpcConvOnmetal::GrpcToDpFwallPort(grpc_filter.udp().dst_port_lower(), &dp_ports->dst_port.lower))
			return "Invalid udp.dst_port_lower";
		if (!GrpcConvOnmetal::GrpcToDpFwallPort(grpc_filter.udp().src_port_upper(), &dp_ports->src_port.upper))
			return "Invalid udp.src_port_upper";
		if (!GrpcConvOnmetal::GrpcToDpFwallPort(grpc_filter.udp().dst_port_upper(), &dp_ports->dst_port.upper))
...
void CreateFirewallRuleCallOnmetal::ParseReply(struct dpgrpc_reply* reply)
{
	reply_.set_rule_id(&reply->fwrule.rule.rule_id, sizeof(reply->fwrule.rule.rule_id));
}

const char* DeleteFirewallRuleCallOnmetal::FillRequest(struct dpgrpc_request* request)
{
...
void DeleteFirewallRuleCallOnmetal::ParseReply(__rte_unused struct dpgrpc_reply* reply)
{
}

const char* GetFirewallRuleCallOnmetal::FillRequest(struct dpgrpc_request* request)
{
...
void GetFirewallRuleCallOnmetal::ParseReply(struct dpgrpc_reply* reply)
{
	FirewallRule *rule = new FirewallRule();

	GrpcConvOnmetal::DpToGrpcFwrule(&reply->fwrule.rule, rule);
	reply_.set_allocated_rule(rule);
}

const char* ListFirewallRulesCallOnmetal::FillRequest(struct dpgrpc_request* request)
{
...
void ListFirewallRulesCallOnmetal::ParseReply(struct dpgrpc_reply* reply)
{
	...
	FOREACH_MESSAGE(grpc_rule, reply) {
		...
		GrpcConvOnmetal::DpToGrpcFwrule(&grpc_rule->rule, rule);
	}
}


const char* CheckVniInUseCallOnmetal::FillRequest(struct dpgrpc_request* request)
{
	if (!GrpcConvOnmetal::GrpcToDpVniType(request_.type(), &request->vni_in_use.type))
		return "Invalid type";
...
void CheckVniInUseCallOnmetal::ParseReply(struct dpgrpc_reply* reply)
{
	reply_.set_in_use(!!reply->vni_in_use.in_use);
}

const char* ResetVniCallOnmetal::FillRequest(struct dpgrpc_request* request)
{
	if (!GrpcConvOnmetal::GrpcToDpVniType(request_.type(), &request->vni_reset.type))
		return "Invalid type";
	request->vni_in_use.vni = request_.vni();
...
void ResetVniCallOnmetal::ParseReply(__rte_unused struct dpgrpc_reply* reply)
{
}


const char* CaptureStartCallOnmetal::FillRequest(struct dpgrpc_request* request)
{
	...
	if (!GrpcConvOnmetal::StrToIpv6(request_.capture_config().sink_node_ip().address(), &request->capture_start.dst_addr6))
	...
	for (int i = 0; i < request_.capture_config().interfaces_size(); ++i) {
		if (!GrpcConvOnmetal::GrpcToDpCaptureInterfaceType(request_.capture_config().interfaces(i).interface_type(), &request->capture_start.interfaces[i].type))
...
void CaptureStartCallOnmetal::ParseReply(__rte_unused struct dpgrpc_reply* reply)
{
}

const char* CaptureStopCallOnmetal::FillRequest(__rte_unused struct dpgrpc_request* request)
{
	DPGRPC_LOG_INFO("Stopping packet capture");
	return NULL;
}
void CaptureStopCallOnmetal::ParseReply(struct dpgrpc_reply* reply)
{
	reply_.set_stopped_interface_cnt((uint32_t)reply->capture_stop.port_cnt);
}

const char* CaptureStatusCallOnmetal::FillRequest(__rte_unused struct dpgrpc_request* request)
{
	DPGRPC_LOG_INFO("Getting packet capturing operation's status");
	return NULL;
}
void CaptureStatusCallOnmetal::ParseReply(struct dpgrpc_reply* reply)
{
