...
bool StrToPreferredUnderlay(const std::string& str, union dp_ipv6 *dst)
{
	...
	if (!GrpcConvOnmetal::StrToIpv6(preferred_uderlay, dst))
		return false;
...
void DpToGrpcLoadBalancer(const struct dpgrpc_lb *dp_lb, Loadbalancer *grpc_lb)
{
	...
	lb_ip = new IpAddress();
	GrpcConvOnmetal::DpToGrpcAddress(&dp_lb->addr, lb_ip);
...
static void SetupIpAndPrefix(const struct dp_fwall_rule *dp_rule, IpAddress* ip, Prefix* pfx, bool is_source)
{
	...
	GrpcConvOnmetal::DpToGrpcAddress(&rule_ip, ip);
	if (!rule_ip.is_v6)
