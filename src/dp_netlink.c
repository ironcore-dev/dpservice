...
int dp_nl_set_vf_rate(uint32_t pf_if_idx, uint32_t vf_count, uint32_t min_tx_rate, uint32_t max_tx_rate)
{
	...
	struct ifla_vf_rate *rate = (struct ifla_vf_rate *)RTA_DATA(vfrate);
	...
	if (send(sock, &req, req.nl.nlmsg_len, 0) < 0) {
		DPS_LOG_ERR("Cannot send VF rate message", DP_LOG_NETLINK(strerror(errno)));
		goto cleanup;
	}

	// Wait for ACK
	ssize_t reply_len = recv(sock, reply, sizeof(reply), 0);
