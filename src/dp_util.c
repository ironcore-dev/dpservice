...
int dp_set_vf_rate_limit(uint16_t port_id, uint32_t rate)
{
	...
	int res;
	uint32_t vf_order; // store which order VF this is (e.g., 0 for enp59s0f0v0, 1 for enp59s0f0v1, etc.)
	...
	vf_order = (uint32_t)strtoul(vf_num_str, &endptr, 10);
	...
	res = dp_nl_set_vf_rate(pf_port->if_index, vf_order, 0, rate_in_mbits);
	if (DP_FAILED(res)) {
