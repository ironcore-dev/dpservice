...
static const struct option dp_conf_longopts[] = {
	...
	{ "debug-coredumps", 0, 0, OPT_DEBUG_COREDUMPS },
	{ NULL, 0, 0, 0 }
};
...
static bool debug_coredumps = false;
...
bool dp_conf_is_debug_coredumps(void)
{
	return debug_coredumps;
}
...
static inline void dp_argparse_help(const char *progname, FILE *outfile)
{
	fprintf(outfile, "Usage: %s [options]\n"
		...
		"     --debug-coredumps                  Change /proc/self/coredump-flags to 0x7f for complete core-dumps\n"
	, progname);
}
...
static int dp_conf_parse_arg(int opt, const char *arg)
{
	...
	case OPT_DEBUG_COREDUMPS:
		return dp_argparse_store_true(&debug_coredumps);
	default:
