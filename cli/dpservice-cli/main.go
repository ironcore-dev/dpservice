...
func main() {
	...
	if err != nil {
		...
		if strings.Contains(err.Error(), "Unimplemented desc") {
			err := cmd.RenderError("server", os.Stdout, fmt.Errorf("error in gRPC, client and server are probably using different proto version"), output)
			if err != nil {
		...
		if err.Error() == strconv.Itoa(errors.SERVER_ERROR) || strings.Contains(err.Error(), "error code") {
			err := cmd.RenderError("server", os.Stdout, err, output)
			if err != nil {
				fmt.Printf("failed to render error: %v", err)
			}
			os.Exit(errors.SERVER_ERROR)
		...
		err := cmd.RenderError("client", os.Stdout, err, output)
		if err != nil {
			fmt.Printf("failed to render error: %v", err)
		}
