...
def test_grpc_addinterface_already_allocated(prepare_ifaces, grpc_client):
	# Try to add using an existing vm identifier
	grpc_client.expect_error(105).addinterface(VM2.name, VM2.pci, VM2.vni, VM2.ip, VM2.ipv6)
...
def test_grpc_addinterface_bad_interface(prepare_ifaces, grpc_client):
	...
	grpc_client.expect_error(106).addinterface("new_vm", "invalid", VM2.vni, VM2.ip, VM2.ipv6)
...
def test_grpc_addinterface_route_exists(prepare_ifaces, grpc_client):
	...
	grpc_client.expect_error(202).addinterface(VM4.name, VM4.pci, VM4.vni, VM1.ip, VM1.ipv6)

def test_grpc_delinterface_not_found(prepare_ifaces, grpc_client):
	# Try to delete with machine identifer which doesnt exist
	grpc_client.expect_error(131).delinterface("invalid_name")
...
def test_grpc_addroute_route_exists(prepare_ifaces, grpc_client):
	...
	grpc_client.expect_error(202).addroute(vni1, neigh_vni1_ov_ip_route, vni1, neigh_vni1_ul_ipv6)
...
def test_grpc_add_NAT_and_VIP_same_IP(prepare_ifaces, grpc_client):
	...
	grpc_client.expect_error(352).getvip(VM2.name)
	grpc_client.expect_error(453).getnat(VM2.name)
...
def test_grpc_add_list_delVIP(prepare_ifaces, grpc_client):
	...
	grpc_client.expect_error(303).addvip(VM2.name, vip_vip)
	# Try to add to a machine which doesnt exist
	grpc_client.expect_error(301).addvip("invalid_name", vip_vip)
	grpc_client.delvip(VM2.name)
	grpc_client.expect_error(352).getvip(VM2.name)
...
def test_grpc_add_list_delLBVIP(prepare_ifaces, grpc_client):
	...
	grpc_client.expect_error(551).getlb(lb_name)
...
def test_grpc_add_list_delPfx(prepare_ifaces, grpc_client):
	...
	grpc_client.expect_error(172).addprefix(VM2.name, prefix)
	# Try to add/delete to/from a machine which doesnt exist
	grpc_client.expect_error(171).addprefix("invalid_name", prefix)
	grpc_client.expect_error(182).delprefix("invalid_name", prefix)
...
def test_grpc_add_list_delLoadBalancerTargets(prepare_ifaces, grpc_client):
	...
	# Try to add/delete to/from a machine which doesnt exist
	grpc_client.expect_error(171).addlbprefix("invalid_name", lb_ip)
	grpc_client.expect_error(182).dellbprefix("invalid_name", lb_ip)
...
def test_grpc_add_list_delFirewallRules(prepare_ifaces, grpc_client):
	...
	grpc_client.expect_error(603).addfwallrule(VM3.name, "fw0-vm3", src_prefix="1.2.3.4/16", proto="tcp", action="drop")
	grpc_client.addfwallrule(VM3.name, "fw0-vm3", src_prefix="1.2.3.4/16", proto="tcp")
	# Used rule-id
	grpc_client.expect_error(604).addfwallrule(VM3.name, "fw0-vm3", src_prefix="1.2.3.4/16", proto="tcp", action="drop")
	...
	grpc_client.expect_error(652).getfwallrule(VM3.name, "fw0-vm3")
	specs = grpc_client.listfwallrules(VM3.name)
	...
	grpc_client.expect_error(652).getfwallrule(VM3.name, "fw1-vm3")
	specs = grpc_client.listfwallrules(VM3.name)
	assert len(specs) == 0, \
