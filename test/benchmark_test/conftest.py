...
def test_config(request):
	...
	env_config = next((env for env in config['environments'] if env['name'] == test_config_name), None)
	if not env_config:
		raise RuntimeError(f"Failed to get config info for environment with name {test_config_name}")
