...
from benchmark_test_config import init_lb, tear_down_lb
from helper import *
...
def test_loadbalancer_cross_machine(test_mode, test_config, test_min_throughput_sw_lb, test_min_throughput_hw_lb):
	...
	remote_machine_op_dpservice_create_route("hypervisor-1", f"{lb_config.get_ip()}/32", 0, underlying_ip_lb, lb_config.get_vni())
	...
	remote_machine_op_add_ip_addr("vm3", f"{lb_config.get_ip()}/32", "lo", False)

	remote_machine_op_flow_test("vm3", True, lb_config.get_ip(), 1)
	client_output = remote_machine_op_flow_test("vm1", False, lb_config.get_ip(), 1)
	print(client_output)
	...
	remote_machine_op_delete_ip_addr("vm3", f"{lb_config.get_ip()}/32", "lo", False)
	remote_machine_op_dpservice_delete_route("hypervisor-1", f"{lb_config.get_ip()}/32", lb_config.get_vni())
