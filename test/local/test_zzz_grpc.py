...
from helpers import *
from grpc_client import GrpcError, ServiceError
import pytest
...
def test_grpc_client_client_error(prepare_ifaces, grpc_client):
	try:
		...
		assert str(e) == 'gRPC error #100: invalid argument "xxx" for "--vip" flag: ParseAddr("xxx"): unable to parse IP', \
...
def test_grpc_client_grpc_error(prepare_ifaces, grpc_client):
	try:
		...
		print(e)
		assert str(e) == 'gRPC error #100: error creating interface: rpc error: code = InvalidArgument desc = Invalid ipv4_config.primary_address and ipv6_config.primary_address combination', \
			"Invalid error from gRPC client"
		return
	assert False, \
		"gRPC client did not fail as it should"
