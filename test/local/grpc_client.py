...
class GrpcClient:
	...
	def _generateError(self, response):
		...
		if source == "server":
			assert message, \
				f"dpservice not sending any error message for error code {errcode}"
			return ServiceError(errcode, message)
		elif source == "grpc":
			# TODO should we require message here?
			return GrpcError(errcode, message)
		# TODO client source currently missing (I guess should never be expecting failure)
		else:
