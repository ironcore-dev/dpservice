...
def send_ipv4_icmp(dst_ip, pf_name, responder):
	threading.Thread(target=responder, args=(pf_name,)).start()
...
def test_ipv4_in_ipv6(prepare_ipv4, tun_opt, port_redundancy):
	...
	send_ipv4_icmp("192.168.129.5", pf0_tap, responder)
	if port_redundancy:
		send_ipv4_icmp("192.168.129.6", pf1_tap, responder)
