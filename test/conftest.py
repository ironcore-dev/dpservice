...
def prepare_env(request, build_path):
	dp_service_cmd = build_path+"/src/dp_service -l 0,1 --vdev=net_tap0,iface="+pf0_tap+",mac=\""+pf0_mac+"\" "\
		"--vdev=net_tap1,iface="+pf1_tap+",mac=\""+pf1_mac+ "\" --vdev=net_tap2,iface="+vf0_tap+",mac=\""+vf0_mac + "\" --vdev=net_tap3,iface="+vf1_tap+",mac=\""+vf1_mac+ "\"   -- "\
...
def add_machine(build_path):
	add_machine_cmd = build_path+"/test/dp_grpc_client --addmachine vm1 --vni 100 --ipv4 " + vf0_ip + " --ipv6 " + vf0_ipv6
	add_machine_cmd2 = build_path+"/test/dp_grpc_client --addmachine vm2 --vni 100 --ipv4 " + vf1_ip + " --ipv6 " + vf1_ipv6
